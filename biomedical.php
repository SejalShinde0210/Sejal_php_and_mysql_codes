...
$prob = isset($_POST['prob']) ? $_POST['prob'] : 'hardware';
...
$sql = "SELECT * FROM biomedical WHERE floor = '$floor' AND ph_no = '$ph_no' AND prob = '$prob' AND sub_prob = '$sub_prob' AND descrp = '$descrp' AND priority = '$priority' AND time_date = '$time_date' AND emp_id='$emp_id'";
...
if ($count == 1) {
    ...
    $insert = "INSERT INTO biomedical (floor, ph_no, prob, sub_prob, descrp, priority, time_date,photo,emp_id,user_status,administrator_status,admin_assign_time,engine_status,engine_complete_time,closing_status,closing_time,engineer,T1,T2,T3,T4) VALUES ('$floor', '$ph_no', '$prob', '$sub_prob', '$descrp', '$priority', '$time_date','$uploadpath','$emp_id','$submit','$administrator_status','$admin_assign_time','$engine_status','$engine_complete_time','$closing_status','$closing_time','$engineer','$t1','$t2','$t3','$t4')";
    ...
    if ($query) {
        ...
        echo json_encode(array("response" => "error", "error" => mysqli_error($connect)));
    }
}
?>
